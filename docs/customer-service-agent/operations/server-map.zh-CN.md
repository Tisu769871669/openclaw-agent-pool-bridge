...
| 主动消息白名单 | `ACTIVE_STATUS_WHITELIST.json` 维护允许被 agent 主动触达的用户 | `GET/PUT/POST /api/agents/<agent>/active-status-whitelist`；`PUT` 覆盖，`POST` 按主动状态事件增删，同步到 template 和 worker |
