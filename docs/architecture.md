...
| `ActiveStatusWhitelistManager` | Reads and overwrites `ACTIVE_STATUS_WHITELIST.json`, and applies active-status events that add or remove proactive-message targets. |
...
- `ActiveStatusWhitelistManager` 负责 `GET/PUT/POST /api/agents/:agentId/active-status-whitelist`，维护 `ACTIVE_STATUS_WHITELIST.json`；`PUT` 全量覆盖，`POST` 按主动状态事件增删同一个用户；主动发消息前应先确认目标用户在白名单里。
