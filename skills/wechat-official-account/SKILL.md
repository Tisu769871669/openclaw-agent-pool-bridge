...
2. 读取当前 logical agent workspace 根目录下的 `WECHAT_ARTICLE_PERSONA.md`，把它作为公众号文章和配图的人设约束；不要从 `SOUL.md` 推导这类内容运营人设。
3. 明确文章主题、目标读者、参考资料和发布模式。
...
- `WECHAT_ARTICLE_PERSONA.md` 是内容运营专用人设文件，控制公众号文章的口吻、栏目感、商业分寸、选题偏好、配图风格和 image2 prompt 约束。
- 这份文件位于每个 logical agent 的源 workspace，并会同步到 template workspace 和 worker workspace；多个客服共用本 skill，但每个客服可维护自己的公众号内容人设。
- 写公众号文章、设计封面、编写 `image-plan.json` 时，都必须遵守同一份 `WECHAT_ARTICLE_PERSONA.md`。
- 不要把这份人设提示词原文写入 `article.json` 的 `markdown`、`html` 或面向读者的正文。
- 如果文件缺失，先提醒维护者通过 bridge API 写入；在用户明确要求临时继续前，不要自行从 `SOUL.md` 拼接替代人设。
